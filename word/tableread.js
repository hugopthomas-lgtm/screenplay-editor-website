...
export async function castVoices(cast) {
  ...
  for (const c of cast) {
    ...
    const g = (genders[c.name] || 'male').toLowerCase() === 'female' ? 'female' : 'male';
...
export function renderCast(el, read, chosen, balance) {
  ...
  el.innerHTML = `
    ...
    <div class="sv-card tr-cast">
      <div class="tr-row"><span class="tr-who">George<span class="sv-sub">Names, headings and action</span></span><select class="se-input tr-select" data-who="NARRATOR">${voiceOptions(chosen.NARRATOR)}</select><button class="se-mini tr-hear" data-who="NARRATOR" title="Hear this voice">${HEAR}</button></div>
